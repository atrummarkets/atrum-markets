...
export default function WithdrawDialog({ note, onClose }: { note: LiveNote; onClose: () => void }) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(6,7,10,0.86)", display: "flex", alignItems: "center", justifyContent: "center", padding: 32 }}
      onClick={onClose}
    >
      <div
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
          {rungs.map((r) => (
            <button
              ...
              style={{
                padding: "12px 20px",
                border: `1px solid ${amount === r ? color.ivory : color.hairline}`,
                background: amount === r ? color.ivory : "transparent",
                color: amount === r ? color.void : color.pewter,
                borderRadius: 2,
                cursor: "pointer",
